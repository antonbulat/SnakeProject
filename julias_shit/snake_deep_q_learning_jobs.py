...
class MyAgent(object):
    ...
    def test(self, p, snake, time_sec, session, input_l, output_l):
        ...
        start_time = time.time()
        scores = []
        #p.init() TODO look if this changes something
    ...
    def getSmartReward(self, current_state_snake, next_state_snake, reward):
        if reward == -5:  # game over
            ...
            if self.foodIsNearer(current_state_snake, next_state_snake):  # snake goes to apple
                return 0.8*(1-(dis_to_apple/114))

            return 0.2*(1-(dis_to_apple/114))  # snake goes away from apple
    '''
    ...
    def train(self, p, snake,network_path, time_sec, gamma, explored,
        ...
        time_random = 60*60#time_sec // 6
        time_rand_network = (time_sec // 6) * 4
        start_time = time.time()
...
def main():
    snake = game.Snake(width=80, height=80)

    p = PLE(snake, fps=10, display_screen=False,num_steps=3)
    ...
    start_time = time.time()
    session, input_l, output_l = myAgent.train(p, snake,network_path="network_onex",time_sec=60*60*24, gamma=0.5
                                               , explored=0.85,screen_size_x=80,screen_size_y=80,mini_batch_size=30)
